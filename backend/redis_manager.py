...
async def send_to_redis_stream(stream_name: str, message: Dict[str, Union[bytes, Any]], maxlen: int = 1000) -> str:
    ...
    try:
        ...
        return await redis_client.xadd(stream_name, processed_message, maxlen=maxlen, approximate=True)
    except redis.RedisError as e:
        logger.error(f"Error sending message to stream {stream_name}: {e}")
        raise

async def receive_from_redis_stream(stream_name: str, last_id: str = '0', timeout: int = DEFAULT_TIMEOUT) -> list:
...
async def receive_llm_response(timeout: int = DEFAULT_TIMEOUT) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        response = await receive_from_redis_stream('llm_responses', last_id='$', timeout=timeout)
        if response:
